...
export const postProcessPack = function (pack) {
  ...
  for (let name in pack.character) {
    ...
    if (!pack.character[name][4].some(j => j.startsWith("die:"))) pack.character[name][4].push(`die:ext:极略/die/${name}.mp3`);
    pack.character[name][4].push(`${(lib.device || lib.node) ? 'ext:' : 'db:extension-'}极略/${name}.jpg`);
